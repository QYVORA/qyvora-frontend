...
const SecurityTab: React.FC<SecurityTabProps> = ({ securitySummary, securityEvents, summaryStatus = 'loaded', eventsStatus = 'loaded', onRetry }) => {
  ...
  const columns: Column<SecurityEventItem>[] = [
    ...
    { key: 'eventType', header: t('admin.security.colType'), render: (item) => <span className="px-2.5 py-1 rounded-lg bg-accent-dim text-xs font-black uppercase tracking-widest text-accent whitespace-nowrap border border-accent/10">{item.eventType}</span> },
    ...
    { key: 'statusCode', header: t('admin.security.colCode'), render: (item) => <span className={`px-2.5 py-1 rounded-lg text-xs font-black font-mono border ${Number(item.statusCode) >= 400 ? 'bg-danger/10 text-danger border-danger/20' : 'bg-accent/10 text-accent border-accent/20'}`}>{item.statusCode}</span> },
        ...
        <>
          {requestIdFilter && (
            <div className="flex items-center gap-2 rounded-xl border border-accent/30 bg-accent/5 px-4 py-2.5">
              <span className="text-xs font-black uppercase tracking-widest text-text-muted">{t('admin.security.filteredByRequest')}</span>
