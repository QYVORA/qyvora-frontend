    ...
    <div className="space-y-6">
      <div ref={productFormRef} className={`rounded-2xl p-6 md:p-8 space-y-6 transition-all duration-300 border border-border/40 ${productForm.id ? 'bg-accent/5' : 'bg-bg-card'}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="space-y-6">
            <label className="block space-y-2"><span className="text-[9px] font-black uppercase text-text-muted/60 tracking-[0.2em]">{t('admin.market.assetTitle')} *</span><input value={productForm.title} onChange={e => setProductForm(p => ({ ...p, title: e.target.value }))} placeholder={t('admin.market.assetTitlePlaceholder')} className="w-full bg-bg border border-border/60 rounded-xl px-4 py-3.5 text-sm text-text-primary focus:border-accent outline-none transition-all" /></label>
            <label className="block space-y-2"><span className="text-[9px] font-black uppercase text-text-muted/60 tracking-[0.2em]">{t('admin.market.assetDescription')}</span><textarea value={productForm.description} onChange={e => setProductForm(p => ({ ...p, description: e.target.value }))} placeholder={t('admin.market.assetDescriptionPlaceholder')} rows={4} className="w-full bg-bg border border-border/60 rounded-xl px-4 py-3.5 text-sm text-text-primary focus:border-accent outline-none transition-all resize-none" /></label>
          </div>
          <div className="space-y-6">
        ...
        <div className="flex gap-4 pt-4">
          <button onClick={handleSave} disabled={saving} className="btn-primary flex-1 py-4 shadow-lg shadow-accent/20 transition-all active:scale-[0.98] disabled:opacity-50">{saving ? t('admin.market.processing') : productForm.id ? t('admin.market.authorizeUpdate') : t('admin.market.initializeAsset')}</button>
          <button onClick={resetProductForm} className="btn-secondary px-8 transition-all hover:text-accent active:scale-[0.98]">{t('button.purge')}</button>
        </div>
      </div>
        ...
        {products.map(item => (
          <div key={item._id} className="bg-bg-card border border-border/40 rounded-2xl p-5 space-y-4">
            <div className="flex items-start gap-4">
              <div className="w-20 h-20 rounded-xl overflow-hidden shrink-0 shadow-sm border border-border/40"><AuthImage src={item.coverUrl} alt={item.title} width={80} height={80} className="w-full h-full object-cover" loading="lazy" /></div>
            ...
            <div className="grid grid-cols-2 gap-3 pt-2">
              ...
              <button onClick={() => void deleteProduct(item._id)} className="py-3 rounded-xl border border-red-500/20 text-red-400/60 text-[10px] font-black uppercase tracking-widest active:scale-95">{t('admin.market.terminate')}</button>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="hidden md:block bg-bg-card border border-border/40 rounded-2xl overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-left min-w-[640px]">
            <thead className="bg-bg-elevated/50 border-b border-border/40">
              <tr>{[t('admin.market.asset'), t('admin.market.title'), t('admin.market.valuation'), t('admin.market.classification'), t('admin.market.status'), t('admin.market.actions')].map(h => <th key={h} className={`px-6 py-5 text-[10px] font-black uppercase tracking-[0.25em] text-text-muted/60 ${h === t('admin.market.actions') ? 'text-right' : ''}`}>{h}</th>)}</tr>
            </thead>
            <tbody className="divide-y divide-border/20">
              {products.map(item => (
                <tr key={item._id} className="hover:bg-accent-dim/5 transition-colors group">
                   <td className="px-6 py-6"><div className="w-16 h-16 rounded-xl overflow-hidden border border-border/60 group-hover:scale-[1.05] transition-transform duration-500"><AuthImage src={item.coverUrl} alt={item.title} width={64} height={64} className="w-full h-full object-cover" loading="lazy" /></div></td>
                  ...
                  <td className="px-6 py-6"><span className="px-2.5 py-1 rounded-lg bg-accent-dim text-[9px] font-black uppercase tracking-widest text-accent whitespace-nowrap border border-accent/10">{item.type}</span></td>
                   <td className="px-6 py-6"><div className="flex items-center gap-2"><span className={`w-2 h-2 rounded-full ${item.isActive ? 'bg-accent animate-pulse' : 'bg-red-400'}`} /><span className={`text-[9px] font-black uppercase tracking-widest ${item.isActive ? 'text-accent' : 'text-red-400'}`}>{item.isActive ? t('admin.market.operational') : t('badge.offline')}</span></div></td>
                   <td className="px-6 py-6 text-right"><div className="inline-flex gap-3"><button onClick={() => editProduct(item)} aria-label="Edit" className="w-11 h-11 flex items-center justify-center rounded-xl bg-bg border border-border/60 text-text-muted hover:text-accent transition-all active:scale-90 shadow-sm"><IconSearch size={18} /></button><button onClick={() => void deleteProduct(item._id)} aria-label="Delete" className="w-11 h-11 flex items-center justify-center rounded-xl bg-bg border border-red-400/20 text-red-400/60 hover:bg-red-400/10 hover:text-red-400 transition-all active:scale-90 shadow-sm"><Trash2 className="w-4.5 h-4.5" /></button></div></td>
                </tr>
              ))}
            </tbody>
          </table>
