        ...
        <div className="space-y-5">
          {release.status === 'loading' && (
            <div className="flex items-center gap-2 rounded-xl border border-border/20 bg-bg-alt px-4 py-3">
              ...
              <p className="text-xs text-text-muted">Checking the latest release…</p>
            </div>
          )}
          ...
          {release.status === 'ready' && release.version && (
            <div className="flex items-center justify-between rounded-xl border border-border bg-accent/5 px-4 py-2.5">
              <p className="text-xs text-text-secondary">{cfg.note}</p>
              <span className="ml-3 shrink-0 font-mono text-xs font-bold text-accent">
                {release.version}
              </span>
            </div>
          )}
              ...
              {PLATFORMS.map((p) => {
                ...
                return (
                  <button
                    ...
                    className={`inline-flex items-center justify-center gap-1.5 rounded-xl border px-2 py-2.5 text-xs font-black uppercase tracking-widest transition-colors ${
                      platform === p.key
              ...
              <div className="grid grid-cols-2 gap-2">
                {archChoices.map((a) => (
                  <button
                    ...
                    type="button"
                    onClick={() => setArch(a)}
                    className={`rounded-xl border px-2 py-2.5 text-xs font-black uppercase tracking-widest transition-colors ${
                      activeArch === a
                        ? 'border-accent/50 bg-accent/10 text-accent'
                        : 'border-border/50 text-text-muted hover:border-accent/40 hover:text-accent'
                    }`}
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center justify-between gap-3 rounded-xl border border-border/50 bg-bg px-4 py-3">
              ...
              <button
                ...
                className="inline-flex shrink-0 items-center gap-1.5 rounded-lg border border-accent/40 px-3 py-2 text-xs font-black uppercase tracking-widest text-accent transition-colors hover:bg-accent/10 disabled:opacity-50"
              >
