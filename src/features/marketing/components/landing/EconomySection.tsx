      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 md:gap-16 items-center">
        <div className="lg:col-span-5">
          <ScrollReveal>
            ...
            <p className="text-text-secondary text-sm md:text-base mb-6 leading-relaxed">Earn Cyber Points by completing bootcamps and challenges. Use <CpLogo className="w-4 h-4 mx-1" /> in the marketplace and track your progress on the leaderboard.</p>
            <Link to="/cyber-points" className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-accent border border-accent/30 rounded-md px-4 py-2 mb-6 hover:bg-accent-dim transition-all">
              Learn Cyber Points <ArrowRight className="w-3.5 h-3.5" />
            </Link>
            <ul className="flex flex-col space-y-3 mb-8">
              {['Complete bootcamps to earn points', 'Finish challenges to earn more points', 'Build your rank on the leaderboard', 'Spend points in the marketplace'].map((item, i) => (
            ...
            <div className="p-4 md:p-5 bg-accent-dim border border-accent/20 rounded-lg relative overflow-hidden group">
              ...
              <div className="text-2xl md:text-3xl font-bold text-accent font-mono mb-3 inline-flex items-center gap-2">{totalCp.toLocaleString()} <CpLogo className="w-6 h-6" /></div>
        ...
        <div className="lg:col-span-7 grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4">
          ...
          {marketItems.length === 0 ? (
            ...
            marketItems.slice(0, 3).map((prod, idx) => (
              <ScrollReveal key={prod._id || prod.id || idx} delay={idx * 0.1}>
                <div className="card-hsociety p-3 md:p-4 flex flex-col h-full">
                  ...
                  <div className="mt-auto flex flex-col gap-2">
                    ...
                    <Link to="/marketplace" className="w-full py-2 bg-accent text-bg font-bold text-[10px] uppercase tracking-tighter rounded hover:brightness-110 text-center block">Buy with Points</Link>
                  </div>
                </div>
              </ScrollReveal>
            ))
