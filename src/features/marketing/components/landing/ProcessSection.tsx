  ...
  return (
    ...
    <div className="max-w-7xl mx-auto px-4 md:px-8">
      <ScrollReveal className="text-center mb-10 md:mb-20">
        ...
        <p className="text-text-muted text-sm md:text-base max-w-xl mx-auto">Create your operator account, complete phased bootcamp modules, validate skills in challenges, earn points, and unlock the Zero-Day Market.</p>
      </ScrollReveal>
      <div className="mb-10 md:mb-20 -mx-4 px-4 md:mx-0 md:px-0 overflow-x-auto overflow-y-hidden touch-pan-x no-scrollbar">
        <div className="flex md:grid md:grid-cols-5 gap-4 md:gap-5 min-w-max md:min-w-0 snap-x snap-mandatory">
          {[{ title: 'JOIN', desc: 'Create your operator account.' }, { title: 'TRAIN', desc: 'Complete phased bootcamp modules.' }, { title: 'VALIDATE', desc: 'Prove skills in live challenges.' }, { title: 'EARN', desc: 'Collect points for your progress.' }, { title: 'UNLOCK', desc: 'Access the Zero-Day Market.' }].map((step, idx) => (
            <ScrollReveal key={idx} delay={idx * 0.1} className="relative shrink-0 group text-left snap-start w-[76vw] sm:w-[54vw] md:w-auto rounded-xl border border-border bg-bg-card/65 md:bg-transparent md:border-none p-4 md:p-0">
        ...
        {statCards.map((card, idx) => (
          <ScrollReveal key={idx} delay={idx * 0.05} className="relative h-36 sm:h-40 md:h-48 rounded-lg overflow-hidden group border border-border">
            ...
            <div className="absolute bottom-3 left-3 right-3">
              ...
              <div className="flex items-center gap-1 text-[9px] font-bold text-white/70 uppercase tracking-widest mt-0.5"><card.icon className="w-3 h-3 flex-none" /> <span className="truncate">{card.label}</span></div>
            </div>
          </ScrollReveal>
        ))}
      </div>
