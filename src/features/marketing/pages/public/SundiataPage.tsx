...
import { STAGES, RULES, RULE_CATEGORIES, CONFIDENCE_STATES, RISK_THRESHOLDS, GITHUB_URL, BUILD_FROM_SOURCE, QUICK_START, AUTHORIZED_WARNING, SOURCE_EXAMPLES } from '@/features/marketing/data/sundiataData';
...
const REQUIREMENTS = [
  ...
  'Offline identity directory snapshots only: no live source or domain required',
];
...
const SundiataPage = () => {
  return (
    <ToolDocPage
      toolName="sundiata"
      seoTitle="sundiata - QYVORA"
      seoDescription="sundiata, offline identity directory security assessment framework in Go. Identity discovery, authentication and MFA posture, credential exposure, privilege mapping and identity attack paths."
      sections={DOC_SECTIONS}
      githubUrl={GITHUB_URL}
      installLabel="Install"
      onInstall={() => openToolInstall('sundiata')}
    >
      {/* ── Hero ─────────────────────────────────────────────────────────── */}
      <ToolDocHero
        toolName="sundiata"
        description="Identity directory security assessment in Go. Analyze offline identity snapshots for discovery, authentication and MFA posture, credential exposure, privilege and relationship mapping, and identity attack paths, with credential material redacted, deterministic rules and transparent risk scoring."
        stats={[
        ...
        actions={
          <>
            <button type="button" onClick={() => openToolInstall('sundiata')} className="btn-primary inline-flex items-center gap-2 px-6 py-2.5">
      ...
      >
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8 items-start">
          <Callout
            ...
            eyebrow="Redacted by design"
            title="Credentials never leave the snapshot"
            className="lg:self-start"
          >
            Credential material is redacted before output and never stored. Live identity source
            collection is not implemented and is refused honestly; sundiata only ever reads the
            directory snapshot you provide.
          </Callout>
          ...
          <div className="rounded-2xl border border-border/50 bg-bg-card p-5 md:p-6 flex flex-col gap-4">
            <div className="flex items-center gap-3">
              ...
              <span className="font-mono text-xs uppercase tracking-widest text-text-muted">redacted by design</span>
            </div>
            {[
              'Credential material is redacted before output and never stored on disk.',
      ...
      <ToolDocSection
        ...
        title="Assess"
        accent="End to End"
        description="From identity discovery to risk scoring, every stage writes to the session and emits a machine-readable event. Each event records exactly what the stage observed."
      >
      ...
      <ToolDocSection
        ...
        title="Identity"
        accent="Rules"
        description="Sundiata applies a fixed, deterministic rule set against the directory snapshot in each session. Exact observation matching means a reusable credential triggers SDT-006, never a weaker neighbor."
      >
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="rounded-2xl border border-border/50 bg-bg-card p-5 md:p-6 space-y-4">
            ...
            <p className="text-xs font-mono text-text-muted leading-relaxed">
              Every finding carries a deterministic ID derived from the rule: SDT-&lt;nnn&gt;, with at least one Evidence record capturing the exact observation that triggered it. Reporting redacts any credential material.
            </p>
            <div>
          ...
          <div className="rounded-2xl border border-border/50 bg-bg-card p-5 md:p-6 space-y-4">
            ...
            <div className="space-y-2.5">
              {[
                ...
                'exposure (0–5): per-category heuristic, where credentials, authentication and attack-path score higher',
              ].map((line) => (
      ...
      <ToolDocSection
        ...
        title="Build &"
        accent="Install"
        description="sundiata ships as source today; build a single static binary with Go 1.26+ and install it to your PATH. Prebuilt release downloads are wired in but not yet published."
      >
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-text-muted mt-1 leading-relaxed">
                    No package manager or prebuilt download needed: a plain Go build produces the sundiata binary.
                  </p>
                </div>
              </div>
              <a
      ...
      <ToolDocSection
        ...
        title="Assess in"
        accent="One Command"
        description="Run the whole offline pipeline with --sim, or point it at your own identity directory with --directory."
      >
        <ToolQuickStart
          command="sundiata assess --sim"
          commandNote="Runs the whole offline identity assessment pipeline against the simulated snapshot."
          output={[
            { label: 'discover', text: 'active-directory - tenant acme - 7 identities, 6 groups' },
            { label: 'enumerate', text: '7 accounts - users, service accounts, app (1 disabled)' },
            { label: 'authenticate', text: '2 passwords never expire, 1 privileged without MFA' },
            { label: 'credentials', text: '1 exposed, 2 rotation overdue - redacted' },
            { label: 'privilege', text: '1 reused credential, 3 sensitive memberships' },
            { label: 'attack-path', text: 'reachable sensitive group through relationship chain' },
            { label: 'risk', text: 'level critical - score 80 - findings 22' },
            { label: 'report', text: 'session stored - report -f json --out report.json' },
          ]}
          usage={QUICK_START}
          footer="Analyze only identity directories you own or have explicit written permission to assess."
        />
      </ToolDocSection>

      {/* ── Related + Continue reading ───────────────────────────────────── */}
      <DocFooterNav currentPath="/sundiata" />
    </ToolDocPage>
  );
};

export default SundiataPage;