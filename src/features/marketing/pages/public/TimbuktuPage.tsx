...
const TimbuktuPage = () => {
  return (
    <ToolDocPage
      toolName="timbuktu"
      seoTitle="timbuktu - QYVORA"
      seoDescription="timbuktu, offline digital forensics investigation framework in Go. Evidence integrity, artifact, filesystem, memory, timeline, log and indicator analysis over offline forensic cases."
      sections={DOC_SECTIONS}
      githubUrl={GITHUB_URL}
      installLabel="Install"
      onInstall={() => openToolInstall('timbuktu')}
    >
      {/* ── Hero ─────────────────────────────────────────────────────────── */}
      <ToolDocHero
        toolName="timbuktu"
        ...
        actions={
          <>
            <button type="button" onClick={() => openToolInstall('timbuktu')} className="btn-primary inline-flex items-center gap-2 px-6 py-2.5">
      ...
      >
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8 items-start">
          <Callout
            ...
            eyebrow="Fixed evidence set"
            title="Deterministic by design"
            className="lg:self-start"
          >
            timbuktu analyzes only the case file you point it at. Live host acquisition is not
            implemented and is refused honestly, so every run stays deterministic against the same
      ...
      <ToolDocSection
        ...
        title="Forensic"
        accent="Rules"
        description="Timbuktu applies a fixed, deterministic rule set against the case in each session. Exact observation matching means a web shell triggers DFI-005, never a weaker neighbor."
      >
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="rounded-2xl border border-border/50 bg-bg-card p-5 md:p-6 space-y-4">
            ...
            <p className="text-[11px] font-mono text-text-muted leading-relaxed">
              Every finding carries a deterministic ID derived from the rule: DFI-&lt;nnn&gt;, with at least one Evidence record capturing the exact observation that triggered it.
            </p>
            <div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1.15fr_1fr] gap-4 md:gap-6">
          <div className="rounded-2xl border border-border/50 bg-bg-card overflow-hidden">
            <div className="flex items-center gap-2 px-4 py-2.5 bg-bg">
              ...
              <span className="ml-2 text-[9px] font-mono text-text-muted">timbuktu, zsh</span>
            </div>
      ...
      <DocFooterNav currentPath="/timbuktu" />
    </ToolDocPage>
  );
};

export default TimbuktuPage;