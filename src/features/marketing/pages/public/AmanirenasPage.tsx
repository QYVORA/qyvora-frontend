...
const AmanirenasPage = () => {
  return (
    <ToolDocPage
      toolName="amanirenas"
      seoTitle="amanirenas - QYVORA"
      seoDescription="amanirenas, offline mobile app security assessment framework in Go. Metadata, static, configuration, API and secrets analysis over app profiles and IPA snapshots with deterministic rules."
      sections={DOC_SECTIONS}
      githubUrl={GITHUB_URL}
      installLabel="Install"
      onInstall={() => openToolInstall('amanirenas')}
    >
      {/* ── Hero ─────────────────────────────────────────────────────────── */}
      <ToolDocHero
        toolName="amanirenas"
      ...
      >
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8 items-start">
          <Callout
            ...
            eyebrow="Nothing executes"
            title="Static analysis by design"
            className="lg:self-start"
          >
            amanirenas analyzes only the snapshot you point it at. There is no live device
            acquisition and no runtime assessment: dynamic capabilities are not implemented and
            are refused honestly, so the tool cannot be pointed at a running app.
          </Callout>
      ...
      <ToolDocSection
        ...
        title="Mobile"
        accent="Rules"
        description="Amanirenas applies a fixed, deterministic rule set against the profile in each session. Exact observation matching means a hardcoded secret triggers AMN-001, never a weaker neighbor."
      >
      ...
      <DocFooterNav currentPath="/amanirenas" />
    </ToolDocPage>
  );
};

export default AmanirenasPage;