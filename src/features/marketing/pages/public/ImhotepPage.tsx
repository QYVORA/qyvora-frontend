...
const ImhotepPage = () => {
  return (
    <ToolDocPage
      toolName="imhotep"
      seoTitle="imhotep - QYVORA"
      seoDescription="imhotep, offline cloud snapshot analysis in Go. IAM, storage exposure, network exposure, container posture, secret redaction and misconfiguration detection without live provider access."
      sections={DOC_SECTIONS}
      githubUrl={GITHUB_URL}
      installLabel="Install"
      onInstall={() => openToolInstall('imhotep')}
    >
      {/* ── Hero ─────────────────────────────────────────────────────────── */}
      <ToolDocHero
        toolName="imhotep"
        ...
        actions={
          <>
            <button type="button" onClick={() => openToolInstall('imhotep')} className="btn-primary inline-flex items-center gap-2 px-6 py-2.5">
      ...
      >
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8 items-start">
          <Callout
            ...
            eyebrow="Never live"
            title="Snapshot analysis by design"
      ...
      <ToolDocSection
        ...
        title="Cloud"
        accent="Rules"
        description="Imhotep applies a fixed, deterministic rule set against the snapshot in each session. Exact observation matching means a publicly writable bucket triggers STG-002, never a weaker neighbor."
      >
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="rounded-2xl border border-border/50 bg-bg-card p-5 md:p-6 space-y-4">
            ...
            <p className="text-[11px] font-mono text-text-muted leading-relaxed">
              Every finding carries a deterministic ID derived from the rule: IAM-&lt;nnn&gt;, STG-&lt;nnn&gt;,
              NET-&lt;nnn&gt;, DBE-&lt;nnn&gt;, CNT-&lt;nnn&gt; or SEC-&lt;nnn&gt;, with at least one Evidence record
              capturing the exact observation that triggered it.
            </p>
            <div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1.15fr_1fr] gap-4 md:gap-6">
          <div className="rounded-2xl border border-border/50 bg-bg-card overflow-hidden">
            <div className="flex items-center gap-2 px-4 py-2.5 bg-bg">
              ...
              <span className="ml-2 text-[9px] font-mono text-text-muted">imhotep, zsh</span>
            </div>
          ...
          <div className="rounded-2xl border border-border/50 bg-bg-card p-5 md:p-6 flex flex-col gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[9px] font-mono text-text-muted mt-0.5">Snapshots, findings, report</p>
              </div>
            </div>
            <CodeBlock
      ...
      <DocFooterNav currentPath="/imhotep" />
    </ToolDocPage>
  );
};

export default ImhotepPage;