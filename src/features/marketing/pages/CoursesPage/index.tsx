...
const SKILL_LEVEL_CONFIG: Record<SkillLevel, { labelKey: string; color: string; icon: React.ElementType }> = {
  beginner: { labelKey: 'landing.courses.level.beginner', color: 'text-accent border-accent/30 bg-accent/10', icon: Sparkles },
  intermediate: { labelKey: 'landing.courses.level.intermediate', color: 'text-blue-400 border-blue-400/30 bg-blue-400/10', icon: TrendingUp },
  advanced: { labelKey: 'coursesPage.levels.advanced', color: 'text-red-400 border-red-400/30 bg-red-400/10', icon: GraduationCap },
};
...
const CoursesPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-bg flex flex-col">
      <SEO
        title={t('coursesPage.hero.title')}
        description={t('coursesPage.hero.description')}
      />
      ...
      <PublicHeroSection showGlobe mask="right">
        <span className="inline-flex items-center gap-2 px-4 py-2.5 sm:px-5 sm:py-3 border border-bg/20 bg-bg/10 rounded-lg max-w-full">
          ...
          <span className="font-mono text-[9px] min-[380px]:text-[10px] sm:text-[11px] font-black uppercase tracking-[0.12em] min-[380px]:tracking-[0.14em] sm:tracking-[0.3em] text-bg whitespace-normal">
            <BookOpen className="h-3 w-3 inline-block -mt-0.5 mr-1.5" /> {t('coursesPage.hero.badge')}
          </span>
        </span>
        <h1 className="font-black text-bg leading-[1.08] tracking-tight w-full relative">
          <span className="block whitespace-normal lg:whitespace-nowrap text-[2rem] min-[400px]:text-[2.25rem] sm:text-[2.5rem] md:text-[3rem] lg:text-[2.5rem] xl:text-[3rem] lg:leading-[1.1] xl:leading-[1.05] uppercase">
            QYVORA{' '}
            <span className="text-bg/80">{t('coursesPage.hero.title')}</span>
          </span>
        </h1>
        ...
        <div className="mx-auto max-w-[1600px] w-full px-4 md:px-10 lg:px-12 xl:px-16 py-12 md:py-16 lg:py-20">
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <div className="relative">
              ...
              >
                {sortMode === 'default' ? t('coursesPage.sort.label') : sortMode === 'popular' ? t('coursesPage.sort.popular') : sortMode === 'price-low' ? t('coursesPage.sort.lowestCp') : sortMode === 'price-high' ? t('coursesPage.sort.highestCp') : t('coursesPage.sort.duration')}
              </button>
              {showSort && (
                <>
                  ...
                  <div className="absolute right-0 top-full mt-1 z-50 bg-bg-card border border-border rounded-2xl p-2 min-w-[160px] shadow-xl" role="listbox" aria-label="Sort options">
                    {(['default', 'popular', 'price-low', 'price-high', 'duration'] as SortMode[]).map((mode) => (
                      ...
                      >
                        {mode === 'default' ? t('coursesPage.sortOptions.default') : mode === 'popular' ? t('coursesPage.sortOptions.popular') : mode === 'price-low' ? t('coursesPage.sortOptions.priceLow') : mode === 'price-high' ? t('coursesPage.sortOptions.priceHigh') : t('coursesPage.sortOptions.duration')}
                      </button>
                    ))}
                  </div>
                </>
          ...
          <div className="flex flex-wrap gap-2 mb-10">
            ...
            >
              <Layers className="h-3 w-3" /> {t('landing.courses.filterAll')}
            </button>
          ...
          <div className="mb-6 text-[10px] font-mono text-text-muted">
            {t('coursesPage.resultsCount', { count: filtered.length })}
                          ...
                          <div className="flex items-center gap-2">
                            {course.popular && (
                              <span className="flex items-center gap-1 text-[9px] font-black uppercase tracking-widest text-yellow-500">
                                <TrendingUp className="h-2.5 w-2.5" /> {t('coursesPage.badge.top')}
                              </span>
                            )}
                        ...
                        <div className="flex items-center justify-between pt-2">
                          <div className="flex items-center gap-2">
                            <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-lg text-[9px] font-black uppercase tracking-widest ${skillCfg.color}`}>
                              <SkillIcon className="h-2.5 w-2.5" /> {t(skillCfg.labelKey)}
                            </span>
                          ...
                          <span className="flex items-center gap-1 text-[10px] font-black uppercase tracking-widest text-accent opacity-0 group-hover:opacity-100 transition-opacity">
                            {t('coursesPage.viewCta')} <IconArrowRight className="h-3 w-3" />
                          </span>
                        </div>
                      </div>
                    </Link>
