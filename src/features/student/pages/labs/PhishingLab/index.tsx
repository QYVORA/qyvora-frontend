...
const PhishingLab = () => {
  ...
  const steps = [
    { title: 'Review Email Details', text: `From: ${activeChallenge.emails[0]?.fromName} <${activeChallenge.emails[0]?.from}>\nSubject: ${activeChallenge.emails[0]?.subject}\nAnalyze the sender address, subject line, and email body for red flags.` },
    { title: 'Inspect Email Headers', text: 'Check the Received paths, SPF/DKIM/DMARC authentication results, and Reply-To header to identify spoofing attempts. Use: `curl -I <smtp-server>` or inspect email source in your terminal.' },
    { title: 'Analyze Links & Attachments', text: `The email has ${activeChallenge.emails[0]?.links?.length || 0} links. Verify actual vs display URLs. Look for mismatched domains, typosquatting, and malicious attachments.` },
  ];
  ...
  return (
    <div className="bg-bg min-h-full">
      <div className="mx-auto max-w-[1600px] px-4 md:px-6 lg:px-8 pt-8 pb-20 lg:pb-24">
        ...
        <p className="text-sm text-text-muted font-mono leading-relaxed mb-8">{activeChallenge.description}</p>
          ...
          {steps.map((step, index) => {
            ...
            return (
              <div key={index} className={`rounded-2xl border p-6 transition-all ${
                ...
                <p className="text-base text-text-secondary font-mono leading-relaxed whitespace-pre-line">{step.text}</p>
                {isNextStep && !isCompleted && (
                  <button onClick={() => handleStepComplete(index)} className="btn-primary !rounded-xl !text-[10px] px-5 py-2.5 mt-4">
                    Mark Step Done
                  </button>
                )}
            ...
            <div className="mb-8">
              ...
              <div className="rounded-2xl border border-accent/30 bg-accent/5 p-6">
                ...
                {flagStatus === 'incorrect' && (
                  <div className="flex items-center gap-2 mt-3 text-sm text-red-400 font-mono"><AlertTriangle className="w-4 h-4" />Incorrect flag. Review the analysis and try again.</div>
                )}
              </div>
            </div>
          </>
        ...
        {!allStepsCompleted && (
          <div className="rounded-2xl border border-yellow-400/20 bg-yellow-400/5 p-4 flex items-start gap-2">
            ...
            <p className="text-sm font-mono text-text-muted">Complete all analysis steps to unlock flag submission.</p>
          </div>
        )}
