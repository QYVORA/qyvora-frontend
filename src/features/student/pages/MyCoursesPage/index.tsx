...
import { Clock, ArrowRight, Zap, GraduationCap, Search, BookOpen, CheckCircle2, Play, BarChart3 } from 'lucide-react';
                ...
                return (
                  <ScrollReveal key={course.id} direction="up" amount={0.1} delay={i * 0.05}>
                    <Link
                      ...
                      className="group block overflow-hidden rounded-xl border border-accent/20 bg-bg-card transition-all hover:border-accent/40"
                    >
                      <div className="aspect-[8/5] overflow-hidden bg-bg-elevated relative">
                        <img src={course.coverSvg} alt={course.title} className={`w-full h-full object-cover transition-transform duration-500 group-hover:scale-105 ${isComplete ? 'brightness-75' : ''}`} />
                        {(pct > 0 || isComplete) && (
                      ...
                      <div className="p-5 space-y-3">
                        ...
                        {/* Progress bar (always show for unlocked) */}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {lockedCourses.map((course, i) => {
                ...
                return (
                  <div
                    key={course.id}
                    className="group block overflow-hidden rounded-xl border border-border/20 bg-bg-card/50 opacity-60"
                  >
