    ...
    <div className="min-h-screen bg-bg">
      ...
      <div className="mx-auto max-w-7xl px-4 md:px-8 lg:px-12 py-8 md:py-12">
        <div className="flex items-center justify-between mb-8">
          ...
          <Link
            to="/courses"
            className="hidden md:inline-flex items-center gap-1.5 px-4 py-2 rounded-sm bg-accent text-bg text-[10px] font-black uppercase tracking-widest transition-all hover:brightness-110"
          >
                ...
                return (
                  <ScrollReveal key={course.id} direction="up" amount={0.1} delay={i * 0.05}>
                    <Link
                      ...
                      className="group block overflow-hidden rounded-sm border border-accent/30 bg-bg-card transition-all hover:border-accent/60"
                    >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {lockedCourses.map((course, i) => {
                ...
                return (
                  <div
                    key={course.id}
                    className="group block overflow-hidden rounded-sm border border-border/20 bg-bg-card/50 opacity-60"
                  >
                    <div className="aspect-[8/5] overflow-hidden bg-bg-elevated">
                    ...
                    <div className="p-5 space-y-3">
                      <div className="flex items-center justify-between">
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-sm bg-bg-elevated text-[9px] font-black uppercase tracking-widest text-text-muted border border-border/20">
                          {category?.name}
                        </span>
        ...
        {!loading && availableCourses.length === 0 && (
          <div className="text-center py-20 space-y-4">
            ...
            <Link
              to="/courses"
              className="inline-flex items-center gap-2 px-6 py-3 bg-accent text-bg rounded-sm text-[10px] font-black uppercase tracking-widest transition-all hover:brightness-110"
            >
              Browse Courses <ArrowRight className="h-3 w-3" />
            </Link>
          </div>
