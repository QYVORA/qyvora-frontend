...
const StepCard: React.FC<Props> = ({
  ...
  >
    <StepRenderer
      ...
      afterContent={<div className="mt-8 flex flex-wrap items-start gap-4" onClick={(e) => e.stopPropagation()}><button onClick={() => onGotIt?.(stepNum, !gotIt)} className={`inline-flex min-h-[44px] items-center gap-1.5 px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest border transition-[background-color,border-color,color] duration-[var(--dur-base)] ease-[var(--ease-smooth)] ${gotIt ? 'bg-accent text-on-accent border-accent' : 'border-border text-text-muted hover:border-accent/50 hover:text-accent'}`}><IconCheck size={12} className={gotIt ? '' : 'opacity-50'} />{gotIt ? t('student.bootcampRoom.stepCard.gotIt') : t('student.bootcampRoom.stepCard.markGotIt')}</button></div>}
