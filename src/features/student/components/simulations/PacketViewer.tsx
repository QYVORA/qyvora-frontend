  ...
  return (
    <div className="flex flex-col h-full rounded-2xl border border-border/50 bg-bg-card overflow-hidden">
      <div className="px-4 py-3 bg-bg-elevated border-b border-border/20 flex items-center gap-3">
        <p className="text-xs font-black uppercase tracking-widest text-accent">Packet Viewer</p>
        <input
          type="text"
          value={filter}
          ...
          className="px-2 py-1 rounded bg-bg border border-border/50 text-xs font-mono text-text-primary outline-none w-32"
        />
        <span className="ml-auto text-xs font-mono text-text-muted">{filtered.length} packets</span>
      </div>
          ...
          <div className="border-t border-border/20 p-4 max-h-[250px] overflow-auto">
            ...
            {selected.layers && selected.layers.map((layer, i) => (
              <div key={i} className="mb-2">
                <p className="text-xs font-black uppercase tracking-widest text-accent mb-1">{layer.name}</p>
            ...
            {selected.payload && (
              <div>
                <p className="text-xs font-black uppercase tracking-widest text-accent mb-1">Payload</p>
                <pre className="text-xs font-mono text-text-muted bg-black/40 rounded p-2 whitespace-pre-wrap">{selected.payload}</pre>
              </div>
            )}
          </div>
        )}
