  ...
  return (
    <div className="flex flex-col h-full rounded-2xl border border-border/50 bg-bg-card overflow-hidden">
      <div className="px-4 py-3 bg-bg-elevated border-b border-border/20 flex items-center gap-2">
        ...
        <p className="text-xs font-black uppercase tracking-widest text-accent">Email Client</p>
        <span className="ml-auto text-xs font-mono text-text-muted">{emails.length} messages</span>
      </div>
          ...
          {emails.map(email => (
            ...
            >
              <div className="flex items-center gap-2 mb-1">
                <span className="text-xs font-bold text-text-primary truncate">{email.fromName}</span>
                {email.isPhishing && <AlertTriangle size={10} className="text-warning shrink-0" />}
              </div>
              <p className="text-xs font-mono text-text-muted truncate">{email.subject}</p>
              <p className="text-xs font-mono text-text-muted/50 mt-1">{email.receivedAt}</p>
            </button>
          ))}
        </div>
        ...
        {selected && (
          <div className="flex-1 min-w-0 flex flex-col overflow-auto p-4">
            <div className="mb-4">
              ...
              <p className="text-xs font-mono text-text-muted/50 mt-1">{selected.receivedAt}</p>
            </div>

            {selected.attachments && selected.attachments.length > 0 && (
              <div className="mb-4 p-2 rounded bg-bg-elevated border border-border/20">
                <p className="text-xs font-black uppercase tracking-widest text-text-muted mb-1">Attachments</p>
            ...
            <div className="flex-1 text-xs font-mono text-text-secondary leading-relaxed whitespace-pre-wrap">
              {selected.body}
            </div>
            ...
            <div className="flex items-center gap-2 mt-4 pt-3 border-t border-border/20">
              <button onClick={() => setShowHeaders(!showHeaders)}
                className={`text-xs font-black uppercase tracking-wider px-2 py-1 rounded ${showHeaders ? 'bg-accent/20 text-accent' : 'text-text-muted hover:text-accent'}`}>
                Headers
              </button>
              <button onClick={() => setShowIndicators(!showIndicators)}
                className={`text-xs font-black uppercase tracking-wider px-2 py-1 rounded ${showIndicators ? 'bg-accent/20 text-accent' : 'text-text-muted hover:text-accent'}`}>
                Indicators ({selected.indicators.length})
              </button>
            </div>

            {showHeaders && (
              <div className="mt-3 p-3 bg-black/40 rounded text-xs font-mono">
