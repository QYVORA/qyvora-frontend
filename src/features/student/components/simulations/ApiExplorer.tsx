  ...
  return (
    <div className="flex flex-col h-full rounded-2xl border border-border/50 bg-bg-card overflow-hidden">
      <div className="px-4 py-3 bg-bg-elevated border-b border-border/20">
        <p className="text-xs font-black uppercase tracking-widest text-accent">API Explorer</p>
      </div>
        ...
        <div className="w-[200px] border-r border-border/20 overflow-auto shrink-0">
          {endpoints.map(ep => (
            <button key={ep.id} onClick={() => { setSelectedId(ep.id); setResponse(null); }}
              ...
              <span className={`text-xs font-black uppercase mr-1.5 ${
                ep.method === 'GET' ? 'text-success' :
                ep.method === 'POST' ? 'text-warning' :
                ep.method === 'PUT' ? 'text-info' :
                'text-danger'
              }`}>{ep.method}</span>
              <span className="text-xs font-mono text-text-muted">{ep.path}</span>
            </button>
          ))}
        </div>
          ...
          <div className="flex-1 min-w-0 flex flex-col">
            <div className="p-4 border-b border-border/20">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-xs font-mono text-text-primary">{selected.path}</span>
                <button onClick={sendRequest}
                  className="ml-auto flex items-center gap-1 px-3 py-1.5 rounded-lg bg-accent/10 border border-accent/30 text-xs font-black uppercase tracking-wider text-accent hover:bg-accent/20">
                  <Send size={10} /> Send
                </button>
              </div>
              <p className="text-xs font-mono text-text-muted">{selected.description}</p>

              <div className="mt-3">
                <p className="text-xs font-black uppercase tracking-widest text-text-muted mb-1">Headers</p>
              ...
              {selected.body && (
                <div className="mt-3">
                  <label htmlFor="api-request-body" className="text-xs font-black uppercase tracking-widest text-text-muted mb-1 block">Request Body</label>
                  <textarea id="api-request-body" value={customBody || selected.body} onChange={e => setCustomBody(e.target.value)}
                    className="w-full h-20 bg-black/40 border border-border/50 rounded p-2 text-xs font-mono text-text-primary outline-none resize-none" />
                </div>
              )}
            </div>
              ...
              {response ? (
                <div>
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-xs font-mono text-text-muted">{response.timing}ms</span>
                  </div>
                  ...
                  <pre className="text-xs font-mono text-text-muted bg-black/40 rounded p-3 whitespace-pre-wrap">{response.body}</pre>
                </div>
              ) : (
