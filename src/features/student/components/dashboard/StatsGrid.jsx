...
export function StatsGrid({ currentModule, totalXp, cpBalance, overallProgress }) {
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
      <StatCard label="Current Module" value={currentModule?.title || '—'} sub={currentModule?.status || 'pending'} icon={Target} color="#3A3F8F" className="h-full" />
      ...
      <div className="relative overflow-hidden rounded-2xl min-h-[150px] h-full">
        ...
        <img
          src={CP_COIN}
          alt=""
          className="pointer-events-none absolute right-3 bottom-3 w-12 h-12 opacity-60 drop-shadow-xl"
        />
      </div>
