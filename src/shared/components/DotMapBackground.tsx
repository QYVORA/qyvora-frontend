...
function isLand(lat: number, lng: number): boolean {
  return pip(lat, lng, AFRICA) || pip(lat, lng, N_AMERICA) || pip(lat, lng, S_AMERICA) ||
    pip(lat, lng, EUROPE) || pip(lat, lng, ASIA) || pip(lat, lng, AUSTRALIA);
}
...
const DotMapBackground: React.FC<DotMapBackgroundProps> = ({
  color = '#06B66F',
  opacity = 0.06,
  step = 3,
  className = '',
  ...
  useEffect(() => {
    ...
    const dotR = (step / 180) * H * 0.22;

    for (let lat = 85; lat >= -85; lat -= step) {
      for (let lng = -175; lng <= 175; lng += step) {
