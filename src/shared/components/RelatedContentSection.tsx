  ...
  return (
    <section aria-labelledby="related-content-heading" className={snap ? 'relative w-full min-h-dvh snap-section bg-bg-alt px-3 md:px-4 lg:px-6 pt-24 pb-8 md:pt-28 md:pb-10 lg:pt-32 lg:pb-12 flex flex-col' : ''}>
      <div className={snap ? 'w-full my-auto' : ''}>
        ...
        <h2 id="related-content-heading" className={`${snap ? 'text-xl md:text-2xl mb-6' : 'text-2xl md:text-3xl mb-10'} font-black uppercase tracking-tight`}>
        ...
        <div className={`grid grid-cols-1 ${items.length > 1 ? 'md:grid-cols-2' : ''} gap-8`}>
          {items.map((item) => (
            <Link
              ...
              to={item.to}
              className={`group block terminal-card rounded-2xl border border-border bg-bg-card overflow-hidden transition-[border-color,box-shadow] duration-[var(--dur-slow)] ease-[var(--ease-smooth)] hover:border-accent/40 ${snap ? 'sm:flex' : ''}`}
              style={{ boxShadow: 'var(--card-shimmer)' }}
            >
              <div className={`relative aspect-[16/9] overflow-hidden bg-accent/5 flex items-center justify-center ${snap ? 'max-h-[160px] sm:aspect-auto sm:max-h-none sm:w-[220px] sm:shrink-0' : ''}`}>
                {item.image ? (
                  <img
                    src={item.image}
                    alt={item.title}
                  ...
                  <span className={`${snap ? 'w-16 h-16' : 'w-24 h-24 md:w-28 md:h-28'} flex items-center justify-center text-accent transition-transform duration-500 group-hover:scale-105`}>
                    {item.icon}
                  </span>
                )}
              </div>
              <div className={`${snap ? 'p-5' : 'p-6'}`}>
                {item.badge && (
                  <div className="mb-3">
                ...
                <h3 className={`${snap ? 'text-lg' : 'text-xl'} font-black uppercase tracking-tight text-text-primary mb-2 break-words`}>
                  {item.title}
                </h3>
                {item.subtitle && (
