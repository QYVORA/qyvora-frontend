              ...
              <div className="flex-1 min-w-0">
                ...
                {step.result && (
                  <div className="mt-1 rounded-lg bg-bg-elevated/50 border border-border/10 px-3 py-2">
                    <code className="text-[10px] text-text-secondary break-all">{step.result}</code>
                  </div>
                )}
              </div>
            </div>
        ...
        {commands.map((cmd, idx) => (
          <div key={idx} className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <p className="text-[10px] font-black uppercase tracking-wider text-text-secondary mb-0.5">{cmd.label}</p>
              <div className="rounded-lg bg-bg border border-border/20 px-3 py-1.5">
                <code className="text-xs text-green-400">{cmd.command}</code>
              </div>
              {cmd.description && (
                <p className="text-[9px] font-mono text-text-muted mt-1">{cmd.description}</p>
              )}
            </div>
          </div>
        ))}
