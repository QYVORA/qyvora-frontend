...
- **Landing hero social buttons** — added a social row below the CTAs in the landing main hero (`LandingHeroSection`): a "Follow QYVORA" label (`hero.followUs` i18n key added to `en.json`) + the 5 brand icon buttons (X / LinkedIn / GitHub / YouTube / WhatsApp) reusing the exported `SOCIAL_LINKS` from the footer. Same hover/accent styling as the footer icons.
